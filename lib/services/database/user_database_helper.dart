...
import 'package:masdamas/models/compartido_item.dart';
...
class UserDatabaseHelper {
  ...
  static const String CART_COLLECTION_NAME = "cart";
    static const String UNIDOS_COLLECTION_NAME = "Unidos";
  ...
  Future<bool> addProductToCart(String productId) async {
    ...
    Future<bool> addProductToCompartido(String productId) async {
    String uid = AuthentificationService().currentUser.uid;
    final compartidoCollectionRef = firestore
        .collection(USERS_COLLECTION_NAME)
        .doc(uid)
        .collection(UNIDOS_COLLECTION_NAME);
    final docRef = compartidoCollectionRef.doc(productId);
    final docSnapshot = await docRef.get();
    bool alreadyPresent = docSnapshot.exists;
    if (alreadyPresent == false) {
      docRef.set(CompartidoItem(itemCount: 1).toMap());
    } else {
      docRef.update({CompartidoItem.ITEM_COUNT_KEY: FieldValue.increment(1)});
    }
    return true;
  }
  ...
  Future<num> get compartidoTotal async {
    String uid = AuthentificationService().currentUser.uid;
    final compartidoItems = await firestore
        .collection(USERS_COLLECTION_NAME)
        .doc(uid)
        .collection(CART_COLLECTION_NAME)
        .get();
    num total = 0.0;
    for (final doc in compartidoItems.docs) {
      num itemsCount = doc.data()[CompartidoItem.ITEM_COUNT_KEY];

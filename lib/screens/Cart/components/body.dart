...
import 'package:masdamas/screens/Cart/components/checkout_card.dart';
import 'package:masdamas/screens/product_details/product_details_screen.dart';
import 'package:masdamas/services/data_streams/cart_items_stream.dart';
...
class _BodyState extends State<Body> {
      ...
      builder: (context, snapshot) {
          ...
          if (cartItemsId.length == 0) {
            return Center(
              child: NothingToShowContainer(
                ...
                secondaryMessage: "Tu carrito esta vacío",
              ),
            );
          }
        ...
        return Center(
          child: NothingToShowContainer(
            ...
            secondaryMessage: "Incapaz de conectar a la base de datos",
          ),
        );
      },
    );
          ...
          if (confirmation) {
            if (direction == DismissDirection.startToEnd) {
              ...
              try {
                ...
                if (result == true) {
                  snackbarMessage = "Producto eliminado del carrito correctamente";
                  await refreshPage();
                } else {
                  throw "No se pudo quitar el producto del carrito por motivos desconocidos";
                }
  ...
  Future<void> checkoutButtonCallback() async {
    ...
    final confirmation = await showConfirmationDialog(
      context,
      "Esta es solo una prueba del proyectos, por lo que no hay una interfaz de pago disponible. \n¿Desea continuar con el pedido simulado de productos?",
    );
    ...
    orderFuture.then((orderedProductsUid) async {
      if (orderedProductsUid != null) {
        ...
        try {
          ...
          if (addedProductsToMyProducts) {
            snackbarmMessage = "Productos pedidos con éxito";
          } else {
            throw "No se pudieron realizar pedidos de productos debido a un problema desconocido";
          }
        } on FirebaseException catch (e) {
          Logger().e(e.toString());
          ...
          Scaffold.of(context).showSnackBar(
            SnackBar(
              content: Text(snackbarmMessage ?? "Algo salió mal"),
            ),
          );
        }
      } else {
        throw "Algo salió mal mientras limpiaba el carrito";
      }
      await showDialog(
        context: context,
        builder: (context) {
          return FutureProgressDialog(
            orderFuture,
            message: Text("Realizar el pedido"),
          );
        },
      );
    }).catchError((e) {
      ...
      Scaffold.of(context).showSnackBar(
        SnackBar(
          content: Text("Realizar el pedido"),
        ),
      );
    });
    await showDialog(
